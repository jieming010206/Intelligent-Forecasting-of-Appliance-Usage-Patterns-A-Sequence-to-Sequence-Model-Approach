...
class Decoder(nn.Module):
    def __init__(self, hidden_dim, time_output_dim, appliance_output_dim):
        ...
        self.multihead_attention = nn.MultiheadAttention(embed_dim=hidden_dim, num_heads=5)
    ...
    def forward(self, inputs, encoder_outputs, h_0):
        outputs, h_n = self.gru(inputs, h_0)
        query = outputs.permute(1, 0, 2)
        key = value = encoder_outputs.permute(1, 0, 2)
        attention_outputs, _ = self.multihead_attention(query, key, value)
        outputs = attention_outputs.permute(1, 0, 2)
...
class Model(nn.Module):
    ...
    def forward(self, inputs, targets=None, ratio = 0.3):
        ...
        for t in range(self.output_seq_len):
            ...
            decoder_time_outputs, decoder_appliance_outputs, h_n = self.decoder(decoder_inputs, encoder_outputs, h_n)
